	...
	function escortsPicturesUser($pid) {
		...
		if ($ttl = $pictures_handler->getCount($criteria)) {
			...
			foreach($pictures as $id => $picture) {
				$ii++;
				$jj++;
				$ret['images'][$ii]['url'] = XOOPS_URL.'/modules/escorts/image,thumbnail,'.md5(XOOPS_LICENSE_KEY.date('Ymdhi')).','.$id.'.'.$picture->getVar('extension');
	...
	function escortsPicturesProfile($pid) {
		...
		if ($ttl = $pictures_handler->getCount($criteria)) {
			...
			foreach($pictures as $id => $picture) {
				...
				$ret['images'][$ii]['thumbnail'] = XOOPS_URL.'/modules/escorts/image,resample,'.md5(XOOPS_LICENSE_KEY.date('Ymdhi')).',gallery_thumb,70,id='.$id.'.'.$picture->getVar('extension');
				$ret['images'][$ii]['orginal'] = XOOPS_URL.'/modules/escorts/image,resample'.md5(XOOPS_LICENSE_KEY.date('Ymdhi')).',gallery_large,500,'.$id.'.'.$picture->getVar('extension');
				$ret['images'][$ii]['download'] = XOOPS_URL.'/modules/escorts/image,orginal,='.md5(XOOPS_LICENSE_KEY.date('Ymdhi')).','.$id.'.'.$picture->getVar('extension');
	...
	function escortsPicturesIndex($pid) {
		...
		if ($ttl = $profile_handler->getCount($criteria)) {
			...
			foreach($profiles as $id => $profile) {
				$ii++;
				$jj++;
				$ret['images'][$ii]['url'] = XOOPS_URL.'/modules/escorts/image,default,thumbnail,'.md5(XOOPS_LICENSE_KEY.date('Ymdhi')).','.$profile->getVar('pid').'.jpg';
